...
def api_create_user():
    try:
        ...
        # ic(user_hashed_password) 'scrypt:32768:8:1$KvFumAOwvcy37eK9$926ebe19913d9a6cb3f9655d12f9d55e5d52f8c2918b56153888365688b648bc4e64f6b209b2a18f92d6ad208becbec8e76cd07013abcaa388d296d842b24dc0'
        ...
        cursor.execute(q, (user_pk, user_first_name, user_last_name,
                       user_email, user_hashed_password, user_created_at))
        db.commit()
        ...
        if "company_exception user_first_name" in str(ex):
            ...
            ___tip = render_template(
                "___tip.html", status="error", message=error_message)
...
def api_login():
    try:
        ...
        if not user:
            error_message = "Invalid credentials E"
            ___tip = render_template(
                "___tip.html", status="error", message=error_message)
            return f"""<browser mix-after-begin="#tooltip">{___tip}</browser>""", 400
        ...
        if not check_password_hash(user["user_password"], user_password):
            error_message = "Invalid credentials P"
            ___tip = render_template(
                "___tip.html", status="error", message=error_message)
            return f"""<browser mix-after-begin="#tooltip">{___tip}</browser>""", 400
